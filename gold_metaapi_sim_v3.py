...
OPTIMISATIONS v3 :
  ...
  2. Filtre session NY   : bloque 17h-23h UTC (etait 29% WR)
  3. Filtre RSI strict   : zone 45-55 (hors zone = 18% WR)
  4. SL_MAX_PIPS 6953->4500 : coupe les outliers -130/-140 pips
  5. COOLDOWN_MIN 20->30 : reduit sur-trading en series perdantes
"""
...
RSI_MAX              = 55
SESSION_BLOCK_START  = 17  # v3: bloque session NY 17h-23h UTC (29% WR)
SESSION_BLOCK_END    = 23
...
def is_ny_session_blocked():
    """v3 : bloque session NY 17h-23h UTC (WR backtest = 29%)."""
    now_utc = datetime.now(timezone.utc)
    return SESSION_BLOCK_START <= now_utc.hour < SESSION_BLOCK_END
...
async def analyse_gold(account):
    try:
        ...
        # ── ETAPE 2b : Filtre v3 - Session NY (17h-23h UTC) ──
        if is_ny_session_blocked():
            print("Session NY bloquee (17h-23h UTC, WR=29%) - stop", flush=True)
            return None
...
async def main():
    ...
    print("NY bloc : " + str(SESSION_BLOCK_START) + "h-"
          + str(SESSION_BLOCK_END) + "h UTC | SL_MAX : "
    ...
    send_msg(
        ...
        + "NY bloc : " + str(SESSION_BLOCK_START) + "h-" + str(SESSION_BLOCK_END) + "h UTC\n"
