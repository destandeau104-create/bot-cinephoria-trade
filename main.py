...
Score min   : 50/100 (production = 60)
Calibrage   : ATR=4.635 | RETEST=3.64 | SL_MIN=695 | SL_MAX=6953
"""
...
SL_MAX_PIPS     = 6953    # ATR(4.635) x15  / 0.01 = 6953 pips
RETEST_THRESH   = 3.64    # P30 ecart EMA20 reel RaiseFX
TP_RR_MARKET    = 1.5
TP_RR_SNIPER    = 3.0
COOLDOWN_MIN    = 20      # 20 min sur M1 (vs 30 sur M5)
VOL_THRESHOLD   = 0.70
STOCH_OB        = 80
STOCH_OS        = 20
STOCH_GAP_HIGH  = 70
...
# Score stratifie - simulation plus souple que production
SCORE_MIN       = 50   # production = 60
SCORE_VOL       = 30
SCORE_RSI       = 25
SCORE_DXY_GAP   = 25
SCORE_MOMENTUM  = 20
...
async def analyse_gold(account):
    try:
        ...
        # Score RSI M1 (25 pts) - extremes absolus obligatoires
...
async def main():
    print("="*60, flush=True)
    print("XAU/USD SNIPER v4.3 - SIMULATION METAAPI M1", flush=True)
    print("Broker  : RaiseFX | Symbole : " + SYMBOL, flush=True)
    print("Score   : " + str(SCORE_MIN) + "/100 | ADR : "
          + str(ADR_BLOCK_PCT) + " | DXY gap : " + str(DXY_KD_MIN_GAP), flush=True)
