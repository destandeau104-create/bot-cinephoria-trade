"""
XAU/USD SNIPER v5.1 - MODE SIMULATION METAAPI
=============================================
...
NOUVEAUTES v5.1 (vs v5) :
  1. VERROU SIMULATION    : assert au demarrage + fonctions executoires preemptees
  2. VOLUME hybride 3 lvl : GC=F yfinance > Tick MetaApi > ATR range
  3. DXY hybride          : USDX MetaApi > DX-Y.NYB yfinance
  4. BREAKEVEN            : notification Telegram a +50% TP (Option A)
  5. CREDENTIALS strict   : tous via os.getenv, defaut vide

OPTIMISATIONS v5 :
  - Bougie M1 verte/rouge (recupere du Gold PDF v4, perdu en migration)
  - RSI directionnel BUY>50/SELL<50
  - RETEST_THRESH 4.55 elargi (+freq)
  - Sessions Paris : Pre-Londres + Matin + Pre-NY + Apres-midi

HISTORIQUE WR :
  v2 : score 50  -> WR 48%, DD -$80
  v3 : score 75  -> WR 55.3%, DD -$38
  v4 : RSI dir   -> WR 57.5%, DD -$29
  v5 : bougie M1 -> WR cible 62%+
  v5.1 : + BE    -> WR effectif cible 67-70% (BE economise les LOSS rattrapables)
"""
...
META_ACCT   = os.getenv("META_ACCOUNT_ID", "")
TG_TOKEN    = os.getenv("TELEGRAM_TOKEN",  "")
TG_CHAT     = os.getenv("TELEGRAM_CHAT_ID","")
DXY_SYMBOL  = os.getenv("DXY_SYMBOL",      "USDX")  # symbole RaiseFX pour DXY
SYMBOL      = "Gold"
SIMULATION  = True  # TOUJOURS True - jamais d'ordre reel

# ============================================================
#  VERROU SIMULATION - garantie mathematique
# ============================================================

def _guard_simulation_mode():
    """Verrou de securite : bloque le programme si SIMULATION != True."""
    if SIMULATION is not True:
        raise SystemExit("SECURITE : SIMULATION doit etre True - arret immediat")
    print("[SECURITE] VERROU SIMULATION ACTIF - aucun ordre reel possible", flush=True)

def _forbidden_execution(*args, **kwargs):
    """Sentinelle anti-execution : leve une erreur si appelee."""
    raise SystemExit("SECURITE : tentative d execution d ordre detectee - arret")

# Tracking trade actif (pour BREAKEVEN simule)
_active_trade = None
...
RETEST_THRESH   = 4.55    # v4: elargi 3.64->4.55 (equivalent 2.5$ yfinance, +freq)
TP_RR_MARKET    = 1.5
TP_RR_SNIPER    = 3.0
...
# ── BREAKEVEN v5.1 ───────────────────────────────────────────
BREAKEVEN_TRIGGER_PCT = 0.50   # active BE quand prix atteint 50% du TP
BREAKEVEN_OFFSET_PIPS = 5.0    # SL deplace a entree +5 pips (couvre spread)
...
# ============================================================
#  VOLUME HYBRIDE 3 NIVEAUX (v5.1)
#  1. GC=F yfinance (vrai volume futures CME)
#  2. Tick volume MetaApi (depuis bougie M1)
#  3. ATR range (fallback ultime)
# ============================================================

def _get_volume_futures(ticker="GC=F"):
    """Recupere le volume des futures CME via yfinance."""
    try:
        ...
        print("Volume yfinance " + ticker + " erreur : " + str(e), flush=True)
        return None

def check_volume_hybrid(df_m1, atr_val, ticker_futures="GC=F"):
    """
    Volume hybride 3 niveaux.
    Returns: (volume_ok, source_label)
    """
    # NIVEAU 1 : Futures CME yfinance
    vol_fut = _get_volume_futures(ticker_futures)
    if vol_fut is not None:
        try:
            sig = float(vol_fut.iloc[-2])
            avg = float(vol_fut.iloc[-17:-2].mean())
            if sig > 0 and avg > 0:
                ratio = sig / avg
                print("Volume Futures " + ticker_futures + " sig="
                      + str(round(sig,0)) + " avg=" + str(round(avg,0))
                      + " ratio=" + str(round(ratio,2)), flush=True)
                if ratio >= VOL_THRESHOLD:
                    return True, "Futures " + ticker_futures
                else:
                    print("Volume Futures insuffisant - essai niveau 2", flush=True)
        except: pass

    # NIVEAU 2 : Tick volume MetaApi (depuis bougie M1)
    try:
        if df_m1 is not None and "Volume" in df_m1.columns and len(df_m1) >= 17:
            tick_sig = float(df_m1["Volume"].iloc[-2])
            tick_avg = float(df_m1["Volume"].iloc[-17:-2].mean())
            if tick_sig > 0 and tick_avg > 0:
                ratio = tick_sig / tick_avg
                print("Volume Tick MetaApi sig=" + str(round(tick_sig,0))
                      + " avg=" + str(round(tick_avg,0))
                      + " ratio=" + str(round(ratio,2)), flush=True)
                if ratio >= VOL_THRESHOLD:
                    return True, "Tick MetaApi"
                else:
                    print("Volume Tick insuffisant - essai niveau 3", flush=True)
    except: pass

    # NIVEAU 3 : ATR range fallback
    try:
        h_last = float(df_m1["High"].iloc[-2])
        l_last = float(df_m1["Low"].iloc[-2])
        if atr_val and (h_last - l_last) > atr_val * 0.8:
            print("Volume OK (ATR range fallback)", flush=True)
            return True, "ATR range"
    except: pass

    return False, "insuffisant"

# ============================================================
#  DXY FALLBACK YFINANCE (v5.1)
# ============================================================

def _get_dxy_yfinance():
    """Recupere DX-Y.NYB H1 via yfinance (fallback si USDX MetaApi indispo)."""
    try:
        import yfinance as yf
        df = yf.download("DX-Y.NYB", interval="1h", period="10d",
                         progress=False, auto_adjust=True)
        if df is None or df.empty: return None
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = df.columns.get_level_values(0)
        ...
        print("DXY yfinance erreur : " + str(e), flush=True)
        return None

# ============================================================
#  BREAKEVEN v5.1 - Notification simulee Option A
# ============================================================

def check_breakeven(signal, current_high, current_low):
    """
    Verifie si le breakeven doit etre active a +50% du TP.
    Mode SIMULATION : envoie une notification, n execute aucun ordre.
    Returns: (be_triggered, new_sl_simule)
    """
    ...
    if direction == "BUY":
        tp_dist     = tp - entry
        trigger_lvl = entry + tp_dist * BREAKEVEN_TRIGGER_PCT
        if current_high >= trigger_lvl:
            new_sl = round(entry + BREAKEVEN_OFFSET_PIPS * PIP_GOLD, 2)
            return True, new_sl, trigger_lvl
    else:
        tp_dist     = entry - tp
        trigger_lvl = entry - tp_dist * BREAKEVEN_TRIGGER_PCT
        if current_low <= trigger_lvl:
            new_sl = round(entry - BREAKEVEN_OFFSET_PIPS * PIP_GOLD, 2)
            return True, new_sl, trigger_lvl

    return False, signal.get("sl_mkt"), None
...
async def analyse_gold(account):
    try:
        ...
        # ── ETAPE 7b : Confirmation couleur bougie M1 (v5) ───
        # Presente dans Gold PDF v4 - perdue en migration MetaApi
        # BUY = bougie verte (close > open) | SELL = bougie rouge
        if direction == "BUY" and p <= o:
            print("Bougie M1 non verte - BUY annule (v5)", flush=True)
            return None
        if direction == "SELL" and p >= o:
            print("Bougie M1 non rouge - SELL annule (v5)", flush=True)
            return None
        print("Bougie M1 OK (" + ("verte" if direction=="BUY" else "rouge") + ")", flush=True)

        # ── ETAPE 8 : SCORE DE CONFLUENCE STRATIFIE ──────────
        score     = 0
        score_log = []

        # Score Volume M1 (30 pts) - HYBRIDE v5.1 : GC=F > TickMetaApi > ATR
        vol_ok, vol_src = check_volume_hybrid(df_m1, atr, ticker_futures="GC=F")
        if vol_ok:
            score += SCORE_VOL
        ...
        if rsi < RSI_MIN or rsi > RSI_MAX:
            print("RSI hors zone neutre (" + str(round(rsi,1))
                  ...
                  + ") - stop", flush=True)
            return None
        # v4: RSI doit confirmer la direction (BUY>50, SELL<50)
        if direction == "BUY" and rsi < 50:
            print("RSI non directionnel BUY (" + str(round(rsi,1)) + "<50) - stop v4", flush=True)
            return None
        if direction == "SELL" and rsi > 50:
            print("RSI non directionnel SELL (" + str(round(rsi,1)) + ">50) - stop v4", flush=True)
            return None
        ...
        # Score DXY (25 pts) - HYBRIDE v5.1 : MetaApi USDX > yfinance DX-Y.NYB
        dxy_k = 0.0; dxy_t = "N/A"; dxy_src = "N/A"
        try:
            # 1er essai : USDX via MetaApi (meme source que les signaux)
            df_dxy = await get_candles(account, DXY_SYMBOL, "1h", 50)
            if df_dxy is not None and len(df_dxy) >= 20:
                dxy_src = "MetaApi " + DXY_SYMBOL
            else:
                # Fallback : DX-Y.NYB via yfinance (DXY officiel ICE)
                print("DXY MetaApi indispo - fallback yfinance", flush=True)
                df_dxy = _get_dxy_yfinance()
                if df_dxy is not None and len(df_dxy) >= 20:
                    dxy_src = "yfinance DX-Y.NYB"

            if df_dxy is not None and len(df_dxy) >= 20:
                k, d = calc_stochastic(df_dxy)
                dxy_k = k
                print("DXY [" + dxy_src + "] K=" + str(round(k,1))
                      + " D=" + str(round(d,1)), flush=True)
...
async def main():
    _guard_simulation_mode()  # VERROU SECURITE v5.1
    print("="*60, flush=True)
    print("XAU/USD SNIPER v5.1 - SIMULATION METAAPI", flush=True)
    print("Broker  : RaiseFX | Symbole : " + SYMBOL, flush=True)
    print("Score   : " + str(SCORE_MIN) + "/100 | RSI : "
          + str(RSI_MIN) + "-" + str(RSI_MAX) + " directionnel", flush=True)
    print("Bougie  : confirmation M1 verte/rouge", flush=True)
    print("Volume  : GC=F > Tick MetaApi > ATR (hybride 3 lvl)", flush=True)
    print("DXY     : " + DXY_SYMBOL + " MetaApi > DX-Y.NYB yfinance", flush=True)
    print("BE      : a +50% TP (notification Telegram)", flush=True)
    print("Sessions: Pre-Londres + Matin + Pre-NY + Apres-midi", flush=True)
    print("ZERO ORDRE REEL - SIMULATION PURE", flush=True)
    print("="*60, flush=True)
    ...
    send_msg(
        "XAU/USD SNIPER v4.3 - SIMULATION M1 [ENGINE v4]\n"
        + "Broker  : RaiseFX | Gold CFD\n"
        + "Score   : " + str(SCORE_MIN) + "/100 (Mom obligatoire)\n"
        + "RSI     : " + str(RSI_MIN) + "-" + str(RSI_MAX) + " directionnel (BUY>50, SELL<50)\n"
        + "Retest  : " + str(RETEST_THRESH) + " pts EMA20\n"
    ...
    while True:
        try:
            ...
            # ── BREAKEVEN v5.1 : check trade actif ──────────────
            global _active_trade
            if _active_trade is not None and not _active_trade["be_active"]:
                try:
                    df_m1_now = await get_candles(account, SYMBOL, "1m", 5)
                    if df_m1_now is not None and len(df_m1_now) >= 2:
                        h_now = float(df_m1_now["High"].iloc[-2])
                        l_now = float(df_m1_now["Low"].iloc[-2])
                        be_ok, new_sl, trigger = check_breakeven(
                            _active_trade["signal"], h_now, l_now)
                        if be_ok:
                            _active_trade["be_active"] = True
                            _active_trade["new_sl"]    = new_sl
                            print("[BE] BREAKEVEN ATTEINT - SL simule -> "
                                  + str(new_sl), flush=True)
                            send_msg("⚡ BREAKEVEN [SIM] XAU/USD\n"
                                     + "Prix a atteint +50% du TP\n"
                                     + "SL simule deplace : " + str(new_sl) + "\n"
                                     + "Entree initiale : "
                                     + str(_active_trade["signal"]["p"]) + "\n"
                                     + "** Action manuelle conseillee sur ton broker **")
                except Exception as e:
                    print("[BE] Erreur check : " + str(e), flush=True)
            ...
            if s:
                ...
                # ── BREAKEVEN v5.1 : enregistrer le trade actif ──
                _active_trade = {
                    "signal":      s,
                    "be_active":   False,
